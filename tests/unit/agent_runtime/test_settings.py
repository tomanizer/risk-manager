...
from agent_runtime.config.settings import Settings, get_settings
...
def test_agent_runtime_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    for var in ("AGENT_RUNTIME_PM_BACKEND", "AGENT_RUNTIME_REVIEW_BACKEND", "AGENT_RUNTIME_CODING_BACKEND"):
        monkeypatch.delenv(var, raising=False)
    get_settings.cache_clear()
    cfg = get_settings()
    assert cfg.agent_runtime.pm_backend == "prepared"
    assert cfg.agent_runtime.review_backend == "prepared"
    assert cfg.agent_runtime.coding_backend == "prepared"
    assert cfg.agent_runtime.coding_pr_backend is None


def test_agent_runtime_backend_override(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AGENT_RUNTIME_CODING_BACKEND", "codex_exec")
    get_settings.cache_clear()
    cfg = get_settings()
    assert cfg.agent_runtime.coding_backend == "codex_exec"
