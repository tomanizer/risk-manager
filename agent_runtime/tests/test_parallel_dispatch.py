...
from agent_runtime.orchestrator.parallel_dispatch import (
    _has_active_lease,
...
class TestRunParallelStep:
    ...
    def test_respects_concurrency_budget(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            snapshot = _make_snapshot()
            with patch("agent_runtime.orchestrator.parallel_dispatch.decide_all_actions", return_value=decisions):
                with patch("agent_runtime.orchestrator.parallel_dispatch._dispatch_one", side_effect=_mock_dispatch):
                    result = run_parallel_step(defaults, snapshot)
