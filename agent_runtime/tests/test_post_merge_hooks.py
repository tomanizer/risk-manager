...
from agent_runtime.orchestrator.post_merge_hooks import run_post_merge_hooks
from agent_runtime.storage.sqlite import initialize_database, load_workflow_events
...
class TestRunPostMergeHooks:
    def test_writes_evidence_events(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("agent_runtime.orchestrator.post_merge_hooks._run_drift_suite", return_value=(0, 10)):
                with patch("agent_runtime.orchestrator.post_merge_hooks._git_head_sha", return_value="abc123"):
    ...
    def test_evidence_event_contains_commit_sha(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("agent_runtime.orchestrator.post_merge_hooks._run_drift_suite", return_value=(0, 5)):
                with patch("agent_runtime.orchestrator.post_merge_hooks._git_head_sha", return_value="deadbeef"):
    ...
    def test_nonzero_drift_findings_logged(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("agent_runtime.orchestrator.post_merge_hooks._run_drift_suite", return_value=(3, 15)):
                with patch("agent_runtime.orchestrator.post_merge_hooks._git_head_sha", return_value="sha1"):
