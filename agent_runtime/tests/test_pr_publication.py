...
from agent_runtime.orchestrator.graph import run_runtime_step
from agent_runtime.orchestrator.pr_publication import (
    PullRequestPublicationResult,
    maybe_publish_completed_coding_run,
)
from agent_runtime.orchestrator.state import NextActionType, RuntimeSnapshot, TransitionDecision, WorkItemSnapshot, WorkItemStage
from agent_runtime.runners.contracts import RunnerDispatchStatus, RunnerExecution, RunnerName, RunnerResult
from agent_runtime.storage.sqlite import WorktreeLeaseRecord, load_workflow_run

from agent_runtime.orchestrator.github_sync import GitHubRepository
...
def test_run_runtime_step_persists_published_pr_number() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch(
            "agent_runtime.orchestrator.graph.decide_next_action",
            ...
            with patch("agent_runtime.orchestrator.graph.build_runner_execution", return_value=execution):
                with patch(
                    "agent_runtime.orchestrator.graph.allocate_worktree",
                    ...
                    with patch("agent_runtime.orchestrator.graph.bind_worktree_to_execution", return_value=bound_execution):
                        with patch("agent_runtime.orchestrator.graph.dispatch_runner_execution", return_value=runner_result):
                            with patch(
                                "agent_runtime.orchestrator.graph.maybe_publish_completed_coding_run",
