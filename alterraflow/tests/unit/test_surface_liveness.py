...
from alterraflow.git_env import GIT_SUBPROCESS_TIMEOUT_SECONDS
from alterraflow.drift.surface_liveness import build_surface_liveness_report
...
def test_surface_liveness_report_detects_missing_repo_module_entrypoint(tmp_path: Path) -> None:
    ...
    (tmp_path / "alterraflow").mkdir(parents=True, exist_ok=True)
    (tmp_path / "alterraflow" / "__init__.py").write_text("", encoding="utf-8")
    (tmp_path / "docs" / "guide.md").write_text(".venv/bin/python -m alterraflow\n", encoding="utf-8")
    ...
    assert finding.related_path == "alterraflow"
...
def test_surface_liveness_report_detects_legacy_import_in_active_code(tmp_path: Path) -> None:
    ...
    app_path.write_text("from alterraflow.legacy.dispatch import run\n", encoding="utf-8")
    ...
    assert "alterraflow.legacy.dispatch" in related_paths


def test_surface_liveness_report_detects_legacy_import_via_from_import(tmp_path: Path) -> None:
    _write_surface_liveness_repo(tmp_path)
    pkg_dir = tmp_path / "alterraflow" / "services"
    pkg_dir.mkdir(parents=True, exist_ok=True)
    (tmp_path / "alterraflow" / "__init__.py").write_text("", encoding="utf-8")
    (pkg_dir / "__init__.py").write_text("", encoding="utf-8")
    (pkg_dir / "runner.py").write_text("from alterraflow import legacy\n", encoding="utf-8")

    report = build_surface_liveness_report(tmp_path)

    legacy_findings = [f for f in report.findings if f.kind == "active_code_imports_legacy_surface"]
    assert len(legacy_findings) == 1
    assert legacy_findings[0].related_path == "alterraflow.legacy"
