...
from alterraflow.drift.architecture_boundaries import build_architecture_boundary_report
...
def test_architecture_boundary_report_detects_runtime_importing_module(tmp_path: Path) -> None:
    _write_boundary_repo(tmp_path)
    runtime_file = tmp_path / "alterraflow" / "runner.py"
