...
from alterraflow.drift.dependency_hygiene import build_dependency_hygiene_report
...
def test_dependency_hygiene_reports_runtime_dependency_declared_only_in_optional_extra(tmp_path: Path) -> None:
    ...
    runtime_file = tmp_path / "alterraflow" / "service.py"
    runtime_file.parent.mkdir(parents=True)
    ...
    assert finding.source_path == "alterraflow/service.py"
