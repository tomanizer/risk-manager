...
Then run the graph from the CLI:
    ...
    python -m alterraflow --langgraph

Or programmatically::

    from alterraflow.orchestrator.langgraph_graph import build_delivery_graph
...
from alterraflow.orchestrator.graph import build_runtime_snapshot
from alterraflow.orchestrator.state import NextActionType, TransitionDecision
from alterraflow.orchestrator.transitions import decide_all_actions
...
def build_delivery_graph(
    ...
    *,
    ...
    if use_checkpointer:
        try:
            from alterraflow.storage.langgraph_checkpointer import SqliteEventCheckpointer
