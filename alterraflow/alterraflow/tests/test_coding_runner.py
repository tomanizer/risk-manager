...
from alterraflow.runners.coding_runner import CodingRunnerInput, build_coding_prompt, dispatch_coding_execution
...
def test_dispatch_coding_execution_codex_backend_returns_completed_outcome() -> None:
    ...
    with patch.dict("os.environ", {"AGENT_RUNTIME_CODING_BACKEND": "codex_exec"}, clear=False):
        get_settings.cache_clear()
        try:
            with patch("alterraflow.runners.coding_backend.subprocess.run", side_effect=fake_run):
                result = dispatch_coding_execution(execution)
        finally:
            get_settings.cache_clear()
