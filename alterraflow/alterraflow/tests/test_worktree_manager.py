...
from alterraflow.git_env import GIT_SUBPROCESS_TIMEOUT_SECONDS, scrub_git_local_env
from alterraflow.handoff_bundle import build_handoff_bundle
from alterraflow.orchestrator.worktree_manager import (
    allocate_worktree,
...
def test_allocate_reuse_and_release_worktree() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        db_path = repo_root / ".alterraflow" / "state.db"
        ...
        assert bound_execution.prompt.startswith("Execution checkout (alterraflow authoritative):")
...
def test_allocate_worktree_cleans_orphaned_worktree_when_lease_insert_loses_race() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("alterraflow.orchestrator.worktree_manager.load_active_worktree_lease", side_effect=fake_load_active):
            with patch(
                "alterraflow.orchestrator.worktree_manager.insert_worktree_lease",
