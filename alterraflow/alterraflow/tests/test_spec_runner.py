...
from alterraflow.orchestrator.transitions import decide_all_actions, decide_next_action
from alterraflow.runners.contracts import RunnerDispatchStatus, RunnerExecution, RunnerName
from alterraflow.runners.spec_runner import SpecRunnerInput, build_spec_prompt, dispatch_spec_execution
from alterraflow.storage.sqlite import WorkflowRunRecord
...
def test_dispatch_spec_execution_codex_backend_returns_completed_outcome() -> None:
    ...
    with patch.dict("os.environ", {"AGENT_RUNTIME_SPEC_BACKEND": "codex_exec"}, clear=False):
        get_settings.cache_clear()
        try:
            with patch("alterraflow.runners.spec_backend.subprocess.run", side_effect=fake_run):
                result = dispatch_spec_execution(execution)
        finally:
            get_settings.cache_clear()
