...
from alterraflow.orchestrator.github_sync import (
...
from alterraflow.orchestrator.graph import build_runtime_snapshot, find_repo_root
from alterraflow.orchestrator.execution import build_runner_execution
from alterraflow.orchestrator.simulations import build_simulation_snapshot, simulation_names
from alterraflow.orchestrator.transitions import decide_next_action
from alterraflow.orchestrator.work_item_registry import load_work_items
from alterraflow.runners.contracts import RunnerDispatchStatus, RunnerName
from alterraflow.runners.dispatch import dispatch_runner_execution
...
def test_build_runtime_snapshot_skips_bootstrap_scans_when_runnable_ready_item_exists(tmp_path: Path) -> None:
    ...
    with (
        patch("alterraflow.orchestrator.graph.load_work_items", return_value=((ready_item,), ())),
        patch("alterraflow.orchestrator.graph.fetch_pull_requests", return_value=((), ())),
        patch("alterraflow.orchestrator.graph.load_workflow_runs", return_value=()),
        patch("alterraflow.orchestrator.graph.build_backlog_materialization_report") as backlog_mock,
        patch("alterraflow.orchestrator.graph.load_prd_bootstrap_candidates") as prd_mock,
    ):
...
def test_build_runtime_snapshot_runs_bootstrap_scans_when_ready_item_is_dependency_blocked(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch("alterraflow.orchestrator.graph.load_workflow_runs", return_value=()),
        patch(
            "alterraflow.orchestrator.graph.build_backlog_materialization_report",
            return_value=backlog_report,
        ) as backlog_mock,
        patch(
            "alterraflow.orchestrator.graph.load_prd_bootstrap_candidates",
...
def test_dispatch_runner_execution_writes_audit_events() -> None:
    ...
    assert all(event.component == "alterraflow.runners.dispatch" for event in events)
...
def test_dispatch_runner_execution_writes_failed_audit_event_on_exception() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        db_path = Path(temp_dir) / "runtime" / "state.db"
        with patch("alterraflow.runners.dispatch.dispatch_pm_execution", side_effect=RuntimeError("boom")):
