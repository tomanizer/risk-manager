...
from alterraflow.runners.review_runner import ReviewRunnerInput, build_review_prompt, dispatch_review_execution
...
def test_dispatch_review_execution_codex_backend_returns_completed_outcome() -> None:
    ...
    with patch.dict("os.environ", {"AGENT_RUNTIME_REVIEW_BACKEND": "codex_exec"}, clear=False):
        get_settings.cache_clear()
        try:
            with patch("alterraflow.runners.review_backend.subprocess.run", side_effect=fake_run):
                result = dispatch_review_execution(execution)
        finally:
            get_settings.cache_clear()
