...
class Settings(BaseSettings):
    ...
    alterraflow: AgentRuntimeConfig = Field(default_factory=AgentRuntimeConfig)

    def is_provider_configured(self, provider: str) -> bool:
        """Return True if the named provider has a non-blank API key set.

        Args:
            provider: a field name on ``Settings`` other than ``"alterraflow"``
        """
    ...
    def configured_providers(self) -> list[str]:
        ...
        future; ``alterraflow`` is excluded because it has no API key.
        """
        return [name for name in type(self).model_fields if name != "alterraflow" and self.is_provider_configured(name)]
