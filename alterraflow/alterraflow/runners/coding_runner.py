...
def build_coding_prompt(input_data: CodingRunnerInput) -> str:
    ...
    prompt += (
        "\nIf dispatched by alterraflow, treat the allocated worktree and checkout context as authoritative."
...
def dispatch_coding_execution(execution: RunnerExecution) -> RunnerResult:
    ...
    if backend == BackendType.CODEX_EXEC:
        return dispatch_codex_coding_execution(
            execution,
            codex_bin=cfg.get_role_codex_bin("coding"),
            model=cfg.get_role_model("coding", backend),
        )
    return RunnerResult(
        runner_name=execution.runner_name,
        work_item_id=execution.work_item_id,
